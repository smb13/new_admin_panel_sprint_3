...
G_STATE_KEY = 'last_genre_modified'

G_IDS_MODIFIED_AFTER = "SELECT id, modified FROM content.genre WHERE modified > %s ORDER BY modified"

FW_IDS_BY_G_IDS = "SELECT DISTINCT fw.id FROM content.film_work fw LEFT JOIN content.genre_film_work gfw " + \
                  "ON gfw.film_work_id = fw.id WHERE gfw.genre_id = ANY(%s::uuid[])"

MOVIES_BY_FW_IDS_G_MODIFIED = "SELECT fw.id as id, fw.rating as imdb_rating, " + \
...
def fetch_changed_genre(cur1, cur2, cur3, next_node: Generator) -> Generator[None, datetime, None]:
    ...
    while last_updated := (yield):
        ...
        cur1.execute(G_IDS_MODIFIED_AFTER, (last_updated,))
        while g_ids := cur1.fetchall():
            logger.info('Fetching film work ids by genres ids %s', g_ids)
            cur2.execute(FW_IDS_BY_G_IDS, ([g_id.get('id') for g_id in g_ids],))
            while fw_ids := cur2.fetchall():
                logger.info('Fetching movies by film work ids %s', fw_ids)
                cur3.execute(MOVIES_BY_FW_IDS_G_MODIFIED, ([fw_id.get('id') for fw_id in fw_ids],))
...
def run_genre_pl() -> None:
    ...
    with (psycopg.connect(dsn, row_factory=dict_row) as conn, ServerCursor(conn, 'fetcher1') as cur1,
        ...
        saver_coro = save_movies(state, G_STATE_KEY)
        ...
        p_fetcher_coro.send(state.get_state(G_STATE_KEY) or str(datetime.min))
