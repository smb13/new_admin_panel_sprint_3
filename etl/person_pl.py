...
PERSON_STATE_KEY = 'last_person_modified'

PERSON_IDS_MODIFIED_AFTER = "SELECT id, modified FROM content.person WHERE modified > %s ORDER BY modified"

FILM_WORK_IDS_BY_PERSON_IDS = """SELECT DISTINCT fw.id FROM content.film_work fw
    LEFT JOIN content.person_film_work pfw ON pfw.film_work_id = fw.id WHERE pfw.person_id = ANY(%s::uuid[])"""

MOVIES_BY_FILM_WORK_IDS_PERSON_MODIFIED = """SELECT fw.id as id, fw.rating as imdb_rating,
...
def fetch_changed_person(cur1, cur2, cur3, next_node: Generator) -> Generator[None, datetime, None]:
    ...
    while last_updated := (yield):
        ...
        cur1.execute(PERSON_IDS_MODIFIED_AFTER, (last_updated,))
        while p_ids := cur1.fetchall():
            logger.info('Fetching film work ids by %s persons ids ', len(p_ids))
            cur2.execute(FILM_WORK_IDS_BY_PERSON_IDS, ([p_id.get('id') for p_id in p_ids],))
            while fw_ids := cur2.fetchall():
                logger.info('Fetching movies by %s film work ids', len(fw_ids))
                cur3.execute(MOVIES_BY_FILM_WORK_IDS_PERSON_MODIFIED, ([fw_id.get('id') for fw_id in fw_ids],))
...
def run_person_pl() -> None:
    ...
    with (psycopg.connect(dsn, row_factory=dict_row) as conn, ServerCursor(conn, 'fetcher1') as cur1,
        ...
        saver_coro = save_movies(state, PERSON_STATE_KEY)
        ...
        p_fetcher_coro.send(state.get_state(PERSON_STATE_KEY) or str(datetime.min))
