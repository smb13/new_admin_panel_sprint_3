...
FILM_WORK_STATE_KEY = 'last_film_work_modified'

FILM_WORK_IDS_MODIFIED_AFTER = "SELECT fw.id FROM content.film_work fw WHERE fw.modified > %s"

MOVIES_BY_FILM_WORK_IDS_FILM_WORK_MODIFIED = """SELECT fw.id as id, fw.rating as imdb_rating,
...
def fetch_changed_film_work(cur1, cur2, next_node: Generator) -> Generator[None, datetime, None]:
    ...
    while last_updated := (yield):
        ...
        cur1.execute(FILM_WORK_IDS_MODIFIED_AFTER, (last_updated,))
        while fw_ids := cur1.fetchall():
            logger.info('Fetching movies by %s film work ids', len(fw_ids))
            cur2.execute(MOVIES_BY_FILM_WORK_IDS_FILM_WORK_MODIFIED, ([fw_id.get('id') for fw_id in fw_ids],))
...
def run_film_work_pl() -> None:
    ...
    with (psycopg.connect(dsn, row_factory=dict_row) as conn, ServerCursor(conn, 'fetcher1') as cur1,
        ...
        saver_coro = save_movies(state, FILM_WORK_STATE_KEY)
        ...
        logger.info('Starting ETL process because of film work modifying')
        fw_fetcher_coro.send(state.get_state(FILM_WORK_STATE_KEY) or str(datetime.min))
